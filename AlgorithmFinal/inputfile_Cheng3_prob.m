...
% G10.nominal_x = Opt_set.dp_x; G11.nominal_x = Opt_set.dp_x; 
 ...
 % Start Mpp_p guess
G1.Mpp_p = pdata.margp(:,5);G2.Mpp_p = G1.Mpp_p;G3.Mpp_p = G1.Mpp_p;
...
% 2 inch^2 in TANA!
G1.expression = {'G = 0.03 - max(abs(U(1,:)));'}; % X OR Y DISPLACEMENT, SHOULD BE X! 4.5*(2.54e-2)
G4.expression = {'G = 0.03 - max(abs(U(2,:)));'}; % X OR Y DISPLACEMENT, SHOULD BE X! 4.5*(2.54e-2)
G2.expression = {' G = 17.23689 - 1e-7*abs(max(F./[A1, A2, A3]));'};
G3.expression = {' G = 17.23689 - 1e-7*abs(min(F./[A1, A2, A3]));'};
...
RBDO_s.RoC_d = 2*(2.54e-2)^2 .*ones(pdata.nd,1);
RBDO_s.RoC_x = 0.01*ones(pdata.nx,1);

RBDO_s.DoE_size_d = 0.01*(2.54e-2)^2.*ones(pdata.nd,1); % 0.1 inch.
RBDO_s.DoE_size_x = Opt_set.target_beta/5 .*ones(pdata.nx,1);
RBDO_s.DoE_size_p = Opt_set.target_beta/5 .*ones(pdata.np,1);
