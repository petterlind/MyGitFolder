...
while flag.outer_conv
    ...
    [alpha_c_new, ~, obj_new, ~,~,~] = Grad_comp(dp, probdata, rbdo_fundata, gfundata, 1, RBDO_settings, lb, 'cost', x_s, rbdo_parameters);
...
%     end
    ...
    for ii = 1:nca %For all active constraints
        ...
        if np > 0 
            ...
            [alpha_p_new, ~, ~, ~, ~, limitstate] = Grad_comp(dp, probdata, rbdo_fundata, gfundata, nr, RBDO_settings, lb, 'p_star', x_s(ii,:,:),rbdo_parameters); 
        ...
        [alpha_x_new, x_new, limit_new, slope_x_new, active_i, ~] = Grad_comp(dp, probdata, rbdo_fundata, gfundata, nr, RBDO_settings, lb_opt, 'limitstate', x_s(ii,:,:), rbdo_parameters);
        ...
        slope_values(nr, index_x:index_x+nx) = slope_x_new;
    end
    ...
    dp_l = dp;
    active_l = logical(active_fix)'; %& update ; %fix
    %active_l = update
    %active_l = update; %fix
...
%     %save figure
%     if flag.debug && flag.inner_conv == 0
%         adress = 'C:\Users\pettlind\Dropbox\KTH\PhD\Article1\Computations\CTRSV3\tex\PseudoCode\fig\';
%         for ij = 1 :fignr
%             savenr = savenr + 1;
%             figure(ij)
%             try
%             suptitle(sprintf('k=%d, l=%d, obj = %d', k, l, obj_new))
%             catch
%                 fprintf('smth off with axes')
%                 offWithAxes=1;
%             end
%             grid on
%             eval(sprintf('print(''%s%d.pdf'',''-dpdf'',''-fillpage'')',adress,savenr))
%            % clf  A6
%         end
%     end
...
%     min_g = min
    ...
    if abs(obj_old - obj_new)/ obj_old < RBDO_settings.tol && min(a) > -10000 && max(abs(dp-dp_old))<1e-3
        flag.outer_conv = 0;
    end
...
fprintf('\n with value of objective function, %1.4f', ObjectiveFunction(dp, rbdo_fundata, gfundata))
fprintf('\n dp = %f ',dp)
fprintf('\n DONE! \n')
...
flag.MC = 0;
% Lista lite optimala parametrar här för framtiden...
if flag.MC == 1
   ...
   % Med nominella gränsvärdet enbart
