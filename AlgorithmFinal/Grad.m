...
case 'parameters'
    ...
    % Probabilistic parameters
        obj.doe_u = Experiment(U_space(obj.Mpp_p, pdata.margp(:,2), pdata.margp(:,3)), RBDO_s.RoC_size_x);
...
gradient(:,1) = const_plane(2:end); % /norm(const_plane(2:end))
...
%             end
    ...
    case 'parameters'
        obj.slope =  obj.alpha_p' * gradient;
        [obj.p_x, obj.p_val] = P_val(obj.doe_x, obj.doe_val, pdata.margp(:,2), obj.alpha_p);
       % obj.p_trial = -obj.nominal_val/obj.slope; % x-space!
        obj.Mpp_p = pdata.margp(:,2) + obj.target_beta * obj.alpha_p.*pdata.marg(:,3); %x-space
